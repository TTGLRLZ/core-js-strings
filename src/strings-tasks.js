...
function removeFirstOccurrences(str, value) {
  ...
  if (indexOf !== -1) {
    const lastIndex = indexOf + value.length;
    const newStr = str.slice(0, indexOf) + str.slice(lastIndex);
    return newStr;
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const indexOf = str.lastIndexOf(value);
  if (indexOf !== -1) {
    const lastIndex = indexOf + value.length;
    const newStr = str.slice(0, indexOf) + str.slice(lastIndex);
    return newStr;
...
function formatTime(minutes, seconds) {
  ...
  const secondFormat = String(seconds).padStart(2, '0');
...
function isPalindrome(str) {
  const re = /[\W_]/g;
  const strLow = str.toLocaleLowerCase().replace(re, '');
  const revers = strLow.split('').reverse().join('');
  return revers === strLow;
}
...
function findLongestWord(sentence) {
  ...
  return split.reduce((a, b) => (b.length > a.length ? b : a));
}
...
function reverseWords(str) {
  ...
  return split.split(' ').reverse().join(' ');
}
